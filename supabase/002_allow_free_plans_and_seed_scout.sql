...
set
  ...
  billing_cycle_months = coalesce(billing_cycle_months, 1)
where segment is null or display_order is null or billing_cycle_months is null;
...
alter table public.saas_plan_catalog
  alter column billing_cycle_months set default 1;
...
update public.saas_plan_catalog set billing_cycle_months = 1 where billing_cycle_months is null;
...
alter table public.saas_plan_catalog
  alter column billing_cycle_months set not null;
...
alter table public.saas_plan_catalog
  drop constraint if exists saas_plan_catalog_billing_cycle_months_check;

alter table public.saas_plan_catalog
  add constraint saas_plan_catalog_billing_cycle_months_check
  check (billing_cycle_months > 0);
...
values
  ...
  ('b2c-card2', 'Hunter Pro', 49, 'BRL', 1, 'b2c', 2, true, 'Assinar Pro'),
  ('b2c-card3', 'Elite', 99, 'BRL', 1, 'b2c', 3, true, 'Assinar Elite'),
  ('b2b-card1', 'Trial', 490, 'BRL', 1, 'b2b', 1, true, 'Teste Grátis'),
  ('b2b-card2', 'Field Ops', 1200, 'BRL', 1, 'b2b', 2, true, 'Falar com Vendas')
